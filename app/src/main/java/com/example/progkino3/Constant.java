package com.example.progkino3;

public class Constant {
    public final static String GEO_CAPITAL = "geo_capital";
    public final static String GEO_COUNTRY = "geo_country";
    public final static String GEO_IMAGE = "geo_image";
    public final static String REVIEW_AUTHOR = "review_author";
    public final static String REVIEW_TIME = "review_time";
    public final static String REVIEW_TEMA = "review_tema";
    public final static String REVIEW_CONTENT = "review_content";
    public final static String IMG_BASA = "chgk3";
    public final static String Color_BASA_Intel = "#D30C2173";
    public final static String Color_BASA_Nastolki = "#D33D00CD";
    public static String no_const_role;
}
